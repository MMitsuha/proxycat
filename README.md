...
mihomo 自身的 REST 控制器（`external-controller`）默认绑定到 `127.0.0.1:9090`，搭配 `metacubexd` 作为 `external-ui`；用户可以在 Settings 里关掉它（写入 `runtime_settings.json` 的 `disableExternalController`，下一次 reload 生效）。它面向最终用户调试，与本仓库的私有 IPC 是两条独立通道。
