...
│   ├── binding.go          Start / Stop / SetTunFd / SetHomeDir / Validate / Version
...
│                           Profile / CommandClient / MemoryMonitor /
│                           AppGroup 路径辅助 / LogEntry / TrafficSnapshot
├── ApplicationLibrary/     SwiftUI 视图：Dashboard / Profiles / Logs / Settings
...
完全对应 sing-box `experimental/libbox.CommandServer` 的方案：
...
mihomo 自身的 REST 控制器（`external-controller`，默认 9090）保留启用，并启用 `metacubexd` 作为 `external-ui`——但那是给最终用户的接口，与本仓库的私有 IPC 是两条独立通道。

## 日志视图

`ApplicationLibrary/LogView.swift` 提供：

1. **等级筛选**：Picker 提供 `Default`（沿用 mihomo 当前运行等级）以及 `Debug / Info / Warning / Error / Silent`。筛选规则为 `entry.level >= cutoff`（mihomo 等级 DEBUG=0 → SILENT=4，所以选 `Warning` 会显示 Warning + Error）。Default 等级从当前激活 profile 的 YAML `log-level` 读取。
...
`Pause / Resume` 冻结快照，便于不被自动滚动顶到底部。`Clear` 同时清空内存环形缓冲区。
...
YAML 中需保留 `tun.enable: true`。但**不要**自己写 `tun.file-descriptor`：fd 由 Network Extension 在运行时通过 `SetTunFd` 注入。同时 `binding.go` 的 `Start` 会强制覆盖以下 TUN 字段以适配 iOS NE 沙箱：
