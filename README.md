...
- `Library/CommandClient.swift` 是 `@Observable`（iOS 17 Observation 框架），封装重连退避（200ms → 5s 上限），SwiftUI 视图直接通过 `@Environment(CommandClient.self)` 订阅。
...
每条 provider 消息都有 10 秒的宿主端超时（`ExtensionProfile.providerMessageTimeout`），扩展卡住时不会让 UI 永远悬挂，而是抛出 `ExtensionProfileError.timeout` 让上层显示一条可恢复的错误。gRPC 流任一边出错时也会主动取消共享 context，避免另一条流在重连周期内继续向宿主推送过期帧。
